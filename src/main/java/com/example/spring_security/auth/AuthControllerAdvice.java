...
public class AuthControllerAdvice extends ResponseEntityExceptionHandler {
    ...
    public ResponseEntity<Object> handleBadCredentialsException(AuthenticationException exception) {
        ErrorResponse errorResponse = new ErrorResponse(ERROR_CODE, HttpStatus.BAD_REQUEST, LocalDateTime.now(), List.of("Username not found"));
    ...
    public ResponseEntity<Object> handleBadCredentialsException(Exception exception) {
        ErrorResponse errorResponse = new ErrorResponse(ERROR_CODE, HttpStatus.BAD_REQUEST, LocalDateTime.now(), List.of(exception.getMessage()));
        return ResponseEntity.badRequest().body(errorResponse);
    }
}